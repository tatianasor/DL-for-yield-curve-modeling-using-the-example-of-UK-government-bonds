...
# 5. SEQUENCES t → t+1
# ===============================
X_train_seq = Z_train_s[:-1]
y_train_seq = Z_train_s[1:]

X_val_seq   = Z_val_s[:-1]
y_val_seq   = Z_val_s[1:]

X_test_seq  = Z_test_s[:-1]
...
# 6. MODEL FACTORY
# ===============================
def build_model(units=64, dropout=0.1, lr=0.001):
    model = Sequential([
        Dense(units, activation='relu', input_shape=(n_components,)),
        Dropout(dropout),
...
# ===============================
# 7. DECODE — факторы → исходные доходности
# ===============================
def decode(Z_pred_s):
...
# ===============================
def run_model_pred(seed, units, dropout, lr, batch_size, X_seq):
    random.seed(seed)
    np.random.seed(seed)
        ...
        for lr in param_grid["lr"]:
            for batch_size in param_grid["batch_size"]:
                Y_pred_gs = run_model_pred(
                    42, units, dropout, lr, batch_size, X_test_seq
                )
                r2 = r2_score(Y_true_universal, Y_pred_gs)
                all_results.append(
                    ((units, dropout, lr, batch_size), round(r2, 4))
                )
                print(f"units={units} drop={dropout} lr={lr} "
                      f"batch={batch_size}  TEST R2={r2:.4f}")
...
for seed in SEEDS:
    ...
    Y_test_pred  = run_model_pred(
        seed, best_units, best_dropout, best_lr, best_batch, X_test_seq
    )
    Y_train_pred = run_model_pred(
        seed, best_units, best_dropout, best_lr, best_batch, X_train_seq
    )
    Y_val_pred   = run_model_pred(
        seed, best_units, best_dropout, best_lr, best_batch, X_val_seq
    )
    ...
    save_seed("PCA_MLP_train", seed, Y_train_true_raw, Y_train_pred)
    save_seed("PCA_MLP_val",   seed, Y_val_true_raw,   Y_val_pred)
    save_seed("PCA_MLP_test",  seed, Y_true_universal, Y_test_pred)

    results_seed.to_excel(
        f"predictions/results_PCA_MLP_seeds_{seed}.xlsx"
    )
    print(f"Saved: predictions/results_PCA_MLP_seeds_{seed}.xlsx")
...
print("\n=== PCA + MLP: mean ± std по 5 seeds ===")
print(summary_stats.to_string(index=False))

summary_df.to_excel(
    "predictions/results_PCA_MLP_all_seeds_summary.xlsx", index=False
)
summary_stats.to_excel(
    "predictions/results_PCA_MLP_mean_std.xlsx", index=False
)
