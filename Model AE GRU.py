...
decoder       = Model(decoder_input, out_dec)
...
def build_model(units, lr):
    model = Sequential([
        GRU(units, input_shape=(LOOKBACK, encoding_dim)),
        Dense(encoding_dim)
    ])
...
for units in param_grid["units"]:
    for lr in param_grid["lr"]:
        for batch_size in param_grid["batch_size"]:
            Y_pred_gs = run_model_pred(42, units, lr, batch_size, X_test_seq)
            r2 = r2_score(Y_true_universal, Y_pred_gs)
            all_results.append(((units, lr, batch_size), round(r2, 4)))
            print(f"units={units} lr={lr} batch={batch_size}  TEST R2={r2:.4f}")
...
for seed in SEEDS:
    ...
    Y_test_pred  = run_model_pred(seed, best_units, best_lr, best_batch, X_test_seq)
    Y_train_pred = run_model_pred(seed, best_units, best_lr, best_batch, X_train_seq)
    Y_val_pred   = run_model_pred(seed, best_units, best_lr, best_batch, X_val_seq)
    ...
    # Сохраняем CSV
    save_seed("AE_GRU_train", seed, Y_train_true_raw, Y_train_pred)
    save_seed("AE_GRU_val",   seed, Y_val_true_raw,   Y_val_pred)
    save_seed("AE_GRU_test",  seed, Y_true_universal, Y_test_pred)

    # Сохраняем Excel с метриками
    results_seed.to_excel(f"predictions/results_AE_GRU_seeds_{seed}.xlsx")
    print(f"Saved: predictions/results_AE_GRU_seeds_{seed}.xlsx")
...
print("\n=== AE + GRU: mean ± std по 5 seeds ===")
print(summary_stats.to_string(index=False))

summary_df.to_excel("predictions/results_AE_GRU_all_seeds_summary.xlsx",
                    index=False)
summary_stats.to_excel("predictions/results_AE_GRU_mean_std.xlsx",
                       index=False)
